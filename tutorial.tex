...
\begin{itemize}
\item 読者からの質問に答えたり、誤植訂正について真摯に対応することは、著者として果たすべき責任だと考えている。
\item 掲示板形式で質問や誤植訂正を募集するという案は、琉球大学の前野昌弘先生のウェブサイト（\url{https://irobutsu.a.la9.jp/mybook/index.html}）を真似ている。
\item 筆者はこれまで Google Sites と Google Forms を組み合わせた「フォーム送信式の掲示板」を利用してきたが、この構成は連携が壊れやすく、運用負担が大きかった。
\end{itemize}

本ノートでは、Google サービスを使わず、より安定した自前運用が可能な構成を紹介する。実装には次の特徴がある。
