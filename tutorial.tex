...
なお、筆者自身はパソコンに詳しいわけではない。実装にあたっては、希望する仕様（匿名投稿可・\LaTeX{} 数式表示・スパム対策・メール通知など）を Claude Code（Anthropic 社の対話型 AI コーディングアシスタント）に伝え、AI からの指示通りに作業を進めるだけで、サイトの構築から公開・運用までを完結することができた。ファイル編集やコマンド実行といった技術的な作業はすべて AI 側が肩代わりしてくれ、人間側はやりたいことを日本語で伝え、画面に表示された手順（アカウント作成、認証、ボタンのクリックなど）を実行するだけでよかった。本ノートと同様の構成を試みる読者にも、AI コーディングアシスタントの活用を強くおすすめしたい。
